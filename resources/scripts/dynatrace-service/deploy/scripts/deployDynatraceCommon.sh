...
function deploy_dt {
    ...
    KEPTN_API=https://api.keptn.$KEPTN_DNS
    KEPTN_API_TOKEN=$(kubectl get secret keptn-api-token -n keptn -ojsonpath={.data.keptn-api-token} | base64 --decode)
    ./setupProblemNotification.sh $DT_TENANT $DT_API_TOKEN $KEPTN_API $KEPTN_API_TOKEN
    ...
    # Create dynatrace-service in Keptn
    ...
    # Create dynatrace-service-distributors in Keptn
    kubectl apply -f ../manifests/dynatrace-service/dynatrace-service-distributors.yaml
    verify_kubectl $? "Deploying dynatrace-service failed."

    # Setup Dashboards and RequestAttributes 
    print_info "Create Dashboards and RequestAttributes"
    ./importDashboardsRequesAttributes.sh $DT_TENANT $DT_API_TOKEN $KEPTN_DNS
    verify_install_step $? "Setup Dashboards and RequestAttributes failed."
    print_info "Setup Setup Dashboards and RequestAttributes done."

    # Expose Bridge via Istio
    print_info "Exposing Keptn Bridge"
    cd ../../../expose-bridge ; ./exposeBridge.sh -y ; cd -
